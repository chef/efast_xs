 ...
 */
unsigned char * efast_xs(unsigned char * string, size_t len, size_t * result_size)
{
	long i;
	size_t s_len = len;
	unsigned char *tmp;
...
static ERL_NIF_TERM escape_nif(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    result_binary_string = efast_xs(input_binary_ro.data, input_binary_ro.size, &result_size);
...
ERL_NIF_INIT(efast_xs, nif_funcs, load, reload, upgrade, unload)
